...
(function () {
	...
	var root = this;
	var previousPopCorn = root.Popcorn;

	var Popcorn;

	if (previousPopCorn) {
		Popcorn = previousPopCorn;
	}
	else{
		Popcorn = root.Popcorn = {}
	}
	/**
	...
	Popcorn.parseSSA = function( data ) {
		...
		while ( ++i < len && lines[ i ] && lines[ i ][ 0 ] !== "[" ) {
			try {
				subs.push(parseSub( lines[ i ], headers ));
	...
	function parseSub( line, headers ) {
		...
		sub = {
			begin: toMilliSeconds( fields[ headers.begin ] ),
			end: toMilliSeconds( fields[ headers.end ] )
		};

		// Invalid time, skip
		if ( sub.begin === -1 || sub.end === -1 ) {
			throw "Invalid time";
		}
